...
class GiftPage: Page{
    ...
    init(pageIndex: Int, startItemIndex: Int, skScene: SKScene){
        super.init(index: pageIndex, skScene: skScene)
        self.GenerateSlots(from: startItemIndex)
    }
    ...
    func GenerateSlots(from index: Int){
        ...
        for y in 0...column{
            for x in 0...row{
                let slot = GiftSlot(index: indexCount)
